...
write_initial_vars_file() {
  ...
  printf '{"updated_at":"","message":"Waiting for Python variables.","vars":[]}\n' >"$vars_file"
}
...
vars_command_for() {
  ...
  printf '%s --vars-file %s --target-pane %s\n' \
...
create_vars_pane() {
  ...
  vars_command="$(vars_command_for "$repl_pane" "$vars_file")"
...
if pane_exists "$existing_repl_pane"; then
  if [[ "$context_command" != "ipython" || ( "$existing_repl_vars_file" == "$vars_file" && "$existing_repl_hook_version" == "$vars_hook_version" ) ]]; then
...
if pane_exists "$existing_vars_pane"; then
  if [[ "$existing_vars_target_pane" == "$repl_pane" && "$existing_vars_ui_version" == "$vars_ui_version" ]]; then
    vars_pane="$existing_vars_pane"
  else
    vars_pane="$(respawn_vars_pane "$existing_vars_pane" "$repl_pane" "$vars_file" || true)"
  fi
else
  vars_pane="$(create_vars_pane "$repl_pane" "$vars_file" || true)"
fi
